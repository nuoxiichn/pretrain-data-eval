...
| 子命令 | pipeline 行 | 描述 |
|--------|------------|------|
| `stats` | 行 1 | 文档数/字符数/token 数、长度分桶、域名/来源/语种分布、时间字段完整性。可选 `--coalesce-stage10` 一次扫描产出 stage10 tokenize summary |
...
- `stats` 默认 `tokenizer.backend: hf`，路径 `/mnt/public/model/huggingface/Qwen3-4B-Base`（vocab 151936，中英都准）。`backend: words` 仅用于 mock smoke，对 CJK 严重失真，**不要在真实数据上用**。
