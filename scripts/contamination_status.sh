...
done=$(ls "$CODE/outputs/stage5/ufw_zh_l3/exact/" 2>/dev/null | wc -l)
echo "  done: $done / 596 ($((done * 100 / 596))%)"

echo "[ZH cascade sample500 (596 files)]"
done=$(ls "$CODE/outputs/stage5/ufw_zh_l3/cascade_sample500/" 2>/dev/null | grep -v aggregated | wc -l)
echo "  done: $done / 596 ($((done * 100 / 596))%)"

echo "[EN cascade sample500 (1168 files)]"
done=$(ls "$CODE/outputs/stage5/ufw_en_l3/cascade_sample500/" 2>/dev/null | grep -v aggregated | wc -l)
...
# 最近 cascade 输出
echo "[最近 ZH cascade]"
tail -3 "$CODE/outputs/stage5/contamination_ufw_zh_l3_cascade_sample500.log" 2>/dev/null | sed 's/^/  /'
echo "[最近 EN cascade]"
tail -3 "$CODE/outputs/stage5/contamination_ufw_en_l3_cascade_sample500.log" 2>/dev/null | sed 's/^/  /'
