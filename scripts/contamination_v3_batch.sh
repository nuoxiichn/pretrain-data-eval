#!/usr/bin/env bash
# Stage 5 v3 — 中文 benchmark 全覆盖版 exact 跑批
# v2 实际只加载 MMLU（v2 log "1 个 benchmark"），v3 加 CMMLU/C-Eval/AGIEval-zh/CMB 4 个中文 benchmark
# 单独输出到 $DATASET_v3，不覆盖 v2 产物
#
# 用法: bash scripts/contamination_v3_batch.sh en|zh
set -uo pipefail

LANG=${1:?用法: contamination_v3_batch.sh en|zh}
...
echo "=== Contamination v3 $DATASET 启动 $(date) config=$CONFIG ===" | tee -a "$LOG"
...
echo "=== Contamination v3 $DATASET 结束: pass=$pass fail=$fail skip=$skip  $(date) ===" | tee -a "$LOG"
